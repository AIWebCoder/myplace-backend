...
class ProfileController extends Controller
{
    ...
    public function apiIndex(Request $request, $username)
    {
        // Find the user by username
        $user = User::where('username', $username)->firstOrFail();

        // Access control
        if (!$user->public_profile && !Auth::check()) {
            return response()->json(['status' => 'error', 'message' => __('Profile access is denied.')], 403);
        }

        // Geoblocking
        if ($this->isGeoLocationBlocked($user)) {
            return response()->json(['status' => 'error', 'message' => __('Profile access is denied.')], 403);
        }

        $hasSub = Auth::check()
            ? Subscription::where('subscriber_id', Auth::id())->where('creator_id', $user->id)->exists()
            : false;

        $postsFilter = $request->get('filter', false);
        $startPage = PostsHelperServiceProvider::getFeedStartPage(PostsHelperServiceProvider::getPrevPage($request));

        // Fetch raw posts (Collection)
        $rawPosts = PostsHelperServiceProvider::getUserPosts($user->id, false, $startPage, $postsFilter, $hasSub);
        PostsHelperServiceProvider::shouldDeletePaginationCookie($request);

        // Manual pagination
        $perPage = $request->input('per_page', 10);
        $page = $request->input('page', 1);
        $postsCollection = collect($rawPosts);
        ...
        $posts = $pagedPosts->values(); // Ensure it's a clean array

        // Profile offer
        $offer = [];
        if ($user->offer && !getSetting('profiles.disable_profile_offers')) {
            $expiringDate = $user->offer->expires_at;
            $currentDate = Carbon::now();
            if ($expiringDate > $currentDate) {
                $offer = [
                    'discountAmount' => [
                        '30' => 100 - (($user->profile_access_price * 100) / $user->offer->old_profile_access_price),
                        '90' => 100 - (($user->profile_access_price_3_months * 100) / max($user->offer->old_profile_access_price_3_months, 1)),
                        '182' => 100 - (($user->profile_access_price_6_months * 100) / max($user->offer->old_profile_access_price_6_months, 1)),
                        '365' => 100 - (($user->profile_access_price_12_months * 100) / max($user->offer->old_profile_access_price_12_months, 1)),
                    ],
                    'daysRemaining' => $expiringDate->diffInDays($currentDate),
                    'expiresAt' => $expiringDate->toDateString(),
                ];
            }
        }
        ...
        if (
            ...
            $recentMedia = PostsHelperServiceProvider::getLatestUserAttachments($user->id, 'image');
        }
        ...
        if ($postsFilter === 'streams') {
            $streams = StreamsServiceProvider::getPublicStreams(['userId' => $user->id, 'status' => 'all']);
            $hasActiveStream = StreamsServiceProvider::getUserInProgressStream(true, $user->id) ? true : false;
        }

        // SEO Bio
        $seoBio = getSetting('profiles.allow_profile_bio_markdown') && $user->bio
            ? strip_tags(GenericHelperServiceProvider::parseProfileMarkdownBio($user->bio))
            : $user->bio;

        $seoDescription = $seoBio
        ...
        return response()->json([
            'status' => 'success',
            'data' => [
                'user' => $user->only(['id', 'username', 'name', 'avatar', 'bio']),
                'hasSub' => $hasSub,
                'posts' => $posts,
                'recentMedia' => $recentMedia,
                'filterTypeCounts' => PostsHelperServiceProvider::getUserMediaTypesCount($user->id),
                'activeFilter' => $postsFilter,
                'offer' => $offer,
                'streams' => $streams,
                ...
                'paginator' => [
                    'next_page_url' => $pagedPosts->nextPageUrl(),
                    'prev_page_url' => $pagedPosts->previousPageUrl(),
                    'current_page' => $pagedPosts->currentPage(),
                    'total' => $pagedPosts->total(),
                    'per_page' => $pagedPosts->perPage(),
                    'hasMore' => $pagedPosts->hasMorePages(),
                ],
            ]
        ]);
    }
