...
class MessengerController extends Controller
{
    ...
    {
        Log::debug('API Messenger request payload:', $request->all());
        ...
        $lastContact = $this->fetchContacts(1);
        if ($lastContact) {
            $lastContactID = $lastContact[0]->receiverID == $user->id ? $lastContact[0]->senderID : $lastContact[0]->receiverID;
        }

        // Handle special case for tipping or unlocking messages
        if ($request->get('tip') || $request->get('messageUnlock')) {
            $transaction = Transaction::query()
        ...
        $availableContacts = $this->getUserSearch($request);
        $followingListID = $user->lists->firstWhere('type', 'following')->id;
        ...
        $response = [
            ...
            'user' => [
                ...
                'user_id' => $user->id,
                'lists' => [
                    'blocked' => $user->lists->firstWhere('type', 'blocked')->id,
                    'following' => $followingListID,
                ],
                'billingData' => [
                    ...
                    'credit' => $user->wallet->total,
                ],
            ],
            'lastContactID' => $lastContactID,
            'unseenMessages' => UserMessage::where('receiver_id', $user->id)->where('isSeen', 0)->count(),
            'availableContacts' => $availableContacts,
        ];
    ...
    public function fetchMessages(Request $request)
    {
        ...
        Log::debug('Fetching messages', ['senderID' => $senderID, 'receiverID' => $receiverID]);
    ...
    public function sendMessage(SaveNewMessageRequest $request)
    {
        ...
        foreach ($receiverIDs as $receiverID) {
            ...
            try {
                $messageResult = $this->sendUserMessage([
                    'senderID' => $senderID,
                ...
                $errors[] = "Failed to send message to {$receiverID}";
                Log::error("Exception while sending message to {$receiverID}", ['exception' => $e]);
            }
        }
        ...
        if ($request->get('attachments')) {
            foreach ($request->get('attachments') as $attachment) {
                ...
                if ($attachmentID) {
                    Log::info("Deleting temporary attachment ID: {$attachmentID}");
                    Attachment::where('id', $attachmentID)->first()?->delete();
                }
            }
        }

        // If single message, return the single message entry
        if (count($receiverIDs) === 1) {
